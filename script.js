...
var OpenMapSurfer_Roads = L.tileLayer(
    'https://{s}.tile.thunderforest.com/outdoors/{z}/{x}/{y}.png?apikey={apikey}',
    {
        ...
        apikey: '3fad6cc7b5cf462f8681b7361b01bc5c',
    }
).addTo(mymap);
...
function addMarker(lat, long) {
    marker2 = L.marker([lat, long]).bindPopup().addTo(mymap).openPopup();
...
function getUserLocation() {
    if (userCoordinates == undefined) {
        if (navigator.geolocation) {
            ...
            alert(
                'API de geolocalização não é suportada pelo seu navegador =('
            );
        }
    } else {
        changeBtnNameAndAnimation();
...
async function getISSPos() {
    ...
    if (stopUpdates == true) {
        ...
        let speed = new Intl.NumberFormat('pt-BR').format(
            data.velocity.toFixed(2)
        ); // com toFixed vazio, corta a vírgula
        ...
        if (userCoordinates == undefined) {
            ...
            kmDistance = new Intl.NumberFormat('pt-BR').format(
